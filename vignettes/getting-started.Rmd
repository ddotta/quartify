...
**Important note about code chunks:** The generated code chunks use standard R chunks with the Quarto options `#| eval: false` and `#| echo: true`, which creates **non-executable** code blocks. This is intentional - `quartify` is designed to create **static documentation** of your R script, not an executable notebook. The code is displayed with syntax highlighting for reading and documentation purposes, but won't be executed when rendering the Quarto document. This approach is ideal for:
