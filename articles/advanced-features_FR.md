...
#### Comment fonctionne apply_styler

Le paramètre `apply_styler` permet de formater **définitivement** vos
fichiers de scripts R selon le guide de style tidyverse.

**Exigences importantes** :

1.  **`use_styler` doit être TRUE** : Le paramètre `apply_styler` ne
    fonctionne que si `use_styler = TRUE`. Si vous définissez
    `apply_styler = TRUE` mais `use_styler = FALSE`, vous recevrez un
    avertissement et le fichier source ne sera pas modifié.

2.  **Le fichier source est modifié AVANT la conversion** : Quand les
    deux paramètres sont TRUE :

    - Votre script R original est formaté sur place avec
    ...
    - Aucun tabset n’est créé (puisque le source est déjà stylisé)

3.  **Modifications permanentes** : Contrairement à `use_styler` seul
    (qui montre seulement des suggestions), `apply_styler` modifie
    réellement votre fichier R sur le disque.

**Comparaison des paramètres** :

| Paramètre              | use_styler | apply_styler | Résultat                                         |
|------------------------|------------|--------------|--------------------------------------------------|
| Suggestions uniquement | TRUE       | FALSE        | Les tabsets montrent le code original vs stylisé |
| Appliquer le formatage | TRUE       | TRUE         | Fichier source modifié, pas de tabsets           |
| Combinaison invalide   | FALSE      | TRUE         | Avertissement affiché, aucun changement          |

**Exemple de flux de travail** :

``` r
# Étape 1 : Prévisualiser d'abord les changements de style (sûr)
rtoqmd("script.R", "apercu.qmd",
       use_styler = TRUE,
       apply_styler = FALSE,  # Sûr : ne modifie pas le source
       render_html = TRUE)

# Étape 2 : Si satisfait des suggestions, les appliquer
rtoqmd("script.R", "output.qmd",
       use_styler = TRUE,
       apply_styler = TRUE)   # Modifie script.R de façon permanente
```

**Cas d’usage pour apply_styler** :

- **Nettoyage de code** : Reformater d’anciens scripts selon les
  standards modernes
- **Cohérence d’équipe** : Assurer que tous les membres suivent le même
  style
- **Formatage pré-commit** : Styler automatiquement le code avant les
  commits
- **Traitement par lots** : Formater plusieurs scripts avec
  [`rtoqmd_dir()`](https://ddotta.github.io/quartify/reference/rtoqmd_dir.md)
  et `apply_styler = TRUE`
- **Préparation pédagogique** : Nettoyer les exemples de code avant de
  les partager avec les étudiants
...
Le fichier de snippets s’ouvrira automatiquement dans RStudio.
Sauvegardez-le pour recharger les snippets immédiatement sans
redémarrer. Vous pouvez ensuite utiliser :
