...
quartify_app <- function(launch.browser = TRUE, port = NULL) {
    ...
    shiny::div(
      ...
      shiny::fluidRow(
        ...
        shiny::column(4, shiny::selectInput("theme", shiny::textOutput("label_theme"),
          choices = c(
            "Default" = "", "Cerulean" = "cerulean", "Cosmo" = "cosmo", "Flatly" = "flatly",
  ...
  server <- function(input, output, session) {
    ...
    output$label_input_file <- shiny::renderText({
      translations[[lang()]]$input_file
    })
    output$label_input_directory <- shiny::renderText({
      translations[[lang()]]$input_directory
    })
    output$label_output_directory <- shiny::renderText({
      translations[[lang()]]$output_directory
    })
    output$button_select_directory <- shiny::renderText({
      translations[[lang()]]$button_select_directory
    })
    output$label_create_book <- shiny::renderText({
      translations[[lang()]]$create_book
    })
    output$label_output_file <- shiny::renderText({
      translations[[lang()]]$output_file
    })
    output$label_html_file <- shiny::renderText({
      translations[[lang()]]$html_file
    })
    output$label_html_file_optional <- shiny::renderText({
      translations[[lang()]]$html_file_optional
    })
    output$label_title <- shiny::renderText({
      translations[[lang()]]$title
    })
    output$label_author <- shiny::renderText({
      translations[[lang()]]$author
    })
    output$label_theme <- shiny::renderText({
      translations[[lang()]]$theme
    })
    output$label_render <- shiny::renderText({
      translations[[lang()]]$render
    })
    output$label_open_html <- shiny::renderText({
      translations[[lang()]]$open_html
    })
    output$label_open_qmd <- shiny::renderText({
      translations[[lang()]]$open_qmd
    })
    output$label_code_fold <- shiny::renderText({
      translations[[lang()]]$code_fold
    })
    output$label_number_sections <- shiny::renderText({
      translations[[lang()]]$number_sections
    })
    output$label_show_source_lines <- shiny::renderText({
      translations[[lang()]]$show_source_lines
    })
      ...
      if (is_directory_mode) {
        if (is.null(selected_dir())) {
          shinyalert::shinyalert(
            title = if (lang() == "fr") "Erreur" else "Error",
            text = if (lang() == "fr") "Veuillez selectionner un repertoire" else "Please select a directory",
            type = "error"
          )
          return()
        }
        ...
        if (is.null(input_file_final) || is.null(output_file_final)) {
          shinyalert::shinyalert(
            title = if (lang() == "fr") "Erreur" else "Error",
            text = if (lang() == "fr") "Veuillez selectionner les fichiers d'entree et de sortie" else "Please select input and output files",
            type = "error"
          )
          return()
        }
