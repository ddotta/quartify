...
- **Before**: Used [`cat()`](https://rdrr.io/r/base/cat.html) to write
  to console in `rtoqmd_dir.R`
- **After**: Replaced with
  [`message()`](https://rdrr.io/r/base/message.html)
- **Impact**: Output can now be suppressed with
  [`suppressMessages()`](https://rdrr.io/r/base/message.html)

### 3. Working Directory Management ✅

- **Before**: [`setwd()`](https://rdrr.io/r/base/getwd.html) called
  without immediate [`on.exit()`](https://rdrr.io/r/base/on.exit.html)
- **After**: Added `on.exit(setwd(old_wd), add = TRUE)` immediately
  after every [`setwd()`](https://rdrr.io/r/base/getwd.html)
- **Files fixed**:
...
- **Before**:
  ...
  wrote to `~/.R/snippets/` by default
- **After**:
  - Added `path` parameter for custom location
  - Examples use [`tempdir()`](https://rdrr.io/r/base/tempfile.html) for
    testing
  - Interactive examples wrapped in `if(interactive())`
- **Impact**: No default writes to user’s home directory in
...
- **Fix**: Added
  [`path.expand()`](https://rdrr.io/r/base/path.expand.html) for all
  file paths in
...
**Version**: 1.0.0 (updated from 0.0.9) **R CMD check**: 0 errors ✔ \| 0
warnings ✔ \| 1 note ✖ (time verification only)

## 📄 Documentation Updated

- **NEWS.md**: Complete changelog for version 1.0.0
- **cran-comments.md**: Updated with all fixes and compliance notes
- **Vignettes**: Added comprehensive `apply_styler` documentation
  (EN/FR)

## 🚀 Ready for CRAN Submission

The package is now fully compliant with CRAN policies and ready for
submission!
...
All 4 major CRAN policy violations have been fixed: 1. ✅ Interactive
functions properly documented 2. ✅ Console output uses message() not
cat() 3. ✅ Working directory always restored with on.exit() 4. ✅ No
default writes to user home directory
